...
def historical_data_with_alerts(kite, data):
    ...
    def calculate_rsi(prices, period=14):
        ...
        For each period, calculate simple averages of gains and losses over the last 14 periods.
        """
        ...
        # Calculate RSI for each period starting from period+1
        for i in range(period, len(prices)):
            # Get the last 14 periods of changes
            period_changes = deltas[i-period:i]
            
            # Calculate gains (up closes) and losses (down closes)
